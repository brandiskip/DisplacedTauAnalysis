    ...
    def process(self, events):
        ...
        if ak.sum(mask_single) > 0:
            ...
            # Signal Logic
                ...
                matched_dr_prop = min_dr_mb2_prop[mask_dr_prop]
                '''
                # Standard Logic
                dr_mb2_std_array = delta_r_mb2_std(single_muons, valid_gen_muons)
                min_dr_mb2_std = ak.min(dr_mb2_std_array, axis=1)
                mask_dr_std = ak.fill_none(min_dr_mb2_std < 0.4, False)
                matched_muons_std = single_muons[mask_dr_std]
                matched_dr_std = min_dr_mb2_std[mask_dr_std]
                '''
                ...
                if len(matched_muons_prop) > 0:
                    ...
                    self.output["single_muon_dR_mb2"].fill(cat="Signal (Propagated)", val=matched_dr_prop)
                '''
                # Fill Standard Matches
                if len(matched_muons_std) > 0:
                    self.output["single_muon_dz_overlay"].fill(cat="Signal (Standard)", val=matched_muons_std.dz)
                    self.output["single_muon_pt"].fill(cat="Signal (Standard)", val=matched_muons_std.pt)
                    self.output["single_muon_eta"].fill(cat="Signal (Standard)", val=matched_muons_std.eta)
                    self.output["single_muon_phi"].fill(cat="Signal (Standard)", val=matched_muons_std.phi)
                    self.output["single_muon_dxy"].fill(cat="Signal (Standard)", val=matched_muons_std.dxy)
                    self.output["single_muon_validDTHits"].fill(cat="Signal (Standard)", val=matched_muons_std.numberOfValidMuonDTHits)
                    self.output["single_muon_validCSCHits"].fill(cat="Signal (Standard)", val=matched_muons_std.numberOfValidMuonCSCHits)
                    self.output["single_muon_validHits"].fill(cat="Signal (Standard)", val=matched_muons_std.numberOfValidMuonHits)
                    self.output["single_muon_dtStations"].fill(cat="Signal (Standard)", val=matched_muons_std.dtStationsWithValidHits)
                    self.output["single_muon_timeAtIpInOut"].fill(cat="Signal (Standard)", val=matched_muons_std.timeAtIpInOut)
                    self.output["single_muon_timeAtIpInOutErr"].fill(cat="Signal (Standard)", val=matched_muons_std.timeAtIpInOutErr)
                    self.output["single_muon_dR_mb2"].fill(cat="Signal (Standard)", val=matched_dr_std)
                '''
        return self.output
