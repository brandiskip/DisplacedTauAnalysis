...
if __name__ == '__main__':
    for sample_name, events in samples.items():
        ...
        events.GenJet = events.GenJet[(events.GenJet.pt > 20) & (abs(events.GenJet) < 2.4)]
        ...
        # Save to JSON file
        json_filename = "jet_score_efficiency_results_isTight.json"
    ...
    # Read the JSON file
    with open("jet_score_efficiency_results_isTight.json", "r") as f:
    ...
    plt.title("(nMatched_gen_vis_taus_highest_score_jet)/(num_vis_gen_taus)[s, $a_{vis , j}, js, L, isTight$]", fontsize=10, pad=15)
    ...
    output_file = "jet_score_efficiency_grid_isTight.pdf"
